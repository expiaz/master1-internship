...
\usepackage{verbatim}         % texte preformat
...
\fancypagestyle{ubs-header-footer}{
  \fancyhead{}
  \fancyfoot{}
  ...
  \cfoot[$if(footer-center)$$footer-center$$else$$institute$$endif$]{$if(footer-center)$$footer-center$$else$$endif$}
...
\pagestyle{ubs-header-footer}

% listings

\definecolor{listing-background}{HTML}{F7F7F7}

\lstdefinestyle{ubs_listing_style}{
  language         = java,
...
\lstset{style=ubs_listing_style}
...
\begin{document}
    \begin{center}
        $if(logo)$
    	\includegraphics[width=$if(logosize)$$logosize$$else$0.3$endif$\textwidth]{$logo$}\\[20pt]
        $endif$
        $if(institute)$
        \large\textbf{$institute$}\\[5pt]
        $endif$
        $if(diploma)$
    	  \large{$diploma$}\\[5pt]
        $endif$
        $if(speciality)$
    	  \large{$speciality$}\\[5pt]
        $endif$
        $if(year)$
        \large\textbf{$year$}
        $endif$
