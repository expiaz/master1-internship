...
Le premier outil, utilisé dans tout nos appareils BLE, est la puce intégrée pour les communications BLE. La recuperation (*sniffing*) et analyse ou modification d'un traffic sans fil etant interdit, ces puces utilisent un *firmware* proprietaire conforme aux roles determinés dans les specifications BLE. Même si il reste possible d'analyser le traffic (notamment en utilisant *Wireshark*^[https://www.wireshark.org/]) entre la puce et un appareil BLE, il n'est pas possible d'etendre les capacites de celle-ci sans modifier le *firmware*.  
